from Classes import Maze, Search_problem, Uninformed_search_agent
...
# Title setup
title_index = 0
titles = ["bfs", "ucs", "dfs", "dls", "idls", "ids"]
...
def get_solutions_by_algorithm(agent, algo, limit):
    if algo == "bfs":
        return agent.breadth_first_search()
    elif algo == "ucs":
        return agent.uniform_cost_search()
    elif algo == "dfs":
        return agent.depth_first_search()
    elif algo == "dls":
        return agent.depth_limited_search(limit)
    elif algo == "idls":
        return agent.iterative_depth_limited_search(limit)
    elif algo == "ids":
        return agent.iterative_deepening_search(limit)
    else:
        return None
...
if __name__ == '__main__':
    ...
    agent = Uninformed_search_agent.agent(search_problem)

    limit = 50
    solution_generator = get_solutions_by_algorithm(agent, algo, limit)
